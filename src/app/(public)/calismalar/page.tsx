...
export default async function CalismalarPage({ searchParams }: { searchParams: SearchParams }) {
                  ...
                  {aktif ? (
                    <div className="mt-3 inline-flex items-center gap-2 text-sm">
                      ...
                      <span className="bg-brand/10 text-brand border-brand/30 rounded-full border px-3 py-1 text-xs tracking-wider uppercase">
                        {calismaKategoriOptions.find((k) => k.value === aktif)?.label ?? aktif}
                      </span>
                      <Link
                        href="/calismalar"
          ...
          {!works || works.length === 0 ? (
            ...
            <div className="grid gap-4 sm:gap-6 md:grid-cols-2">
              {works.map((w) => {
                ...
                return (
                  <Link key={w.slug} href={`/calismalar/${w.slug}`} className="group">
